...
class TestApplyWordDropout(unittest.TestCase):
    def setUp(self):
        self.tokenizer = AutoTokenizer.from_pretrained('tinkoff-ai/ruDialoGPT-small', padding_side='left')
        self.word_dropout = ApplyWordDropout(replace_with=self.tokenizer.convert_tokens_to_ids(self.tokenizer.unk_token),
...
class TestConversationDataset(unittest.TestCase):
    ...
    def test_concat_conv(self):
        ...
        self.assertTrue(torch.all(torch.eq(conv_flat, torch.tensor([50257, 44, 42911, 50258, 44, 1170, 2213, 9497, 35, 50257]))))
