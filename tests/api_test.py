import pytest
import transformers
from src.api.backend import InterfaceModel, PeftModel, PeftConfig
...
def test_predict_with_too_long_history(interface_model):
    interface_model.model_path = "models/rudialogpt-medium-lora-5ep"
    interface_model.chat_history = " ".join(["Sample history"] * 1025)
    _, history = interface_model.predict("Sample text")
